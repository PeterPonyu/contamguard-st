...
class MoleculeRecord:
    ...
    x: float
    y: float
    channel_type: str = "gene"


@dataclass(frozen=True)
class ControlChannelSpec:
    control_types: tuple[str, ...] = ("negative_control", "blank", "codeword")

    def is_control(self, channel_type: str) -> bool:
        return channel_type in self.control_types


@dataclass(frozen=True)
class CellReliabilityReport:
    cell_id: str
...
@dataclass(frozen=True)
class ContaminationSmokeReport:
    reports: tuple[CellReliabilityReport, ...]
